<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:81:"/www/wwwroot/abc.yptsc.cn/public/../application/home/view/merchant/paymobile.html";i:1606930291;}*/ ?>
<!DOCTYPE html>
<html lang="en">
...
<head>
    ...
    <script src="/static/home/js/pay/qrcode.min.js"></script>
    <style>
        #app {
            max-width: 767px;
            margin: 0 auto;
            ...
            <div class="type-list">
                <div v-if="i==0">
                    <div>
                        ...
                        <p style="width:80%;margin:1% auto;font-size:14px;color:red;">
                            此地址为测试地址，请勿充值。否则造成损失由本人承担！
                        </p>
                        <p style="width:80%;margin:1% auto 1%;font-size:14px;">
                            请勿向上述地址充值任何非USDT资产，否则资产将不可找回。您充值至上述地址后，需要整个网络节点的确认，12次网络确认后到账。最小充值金额：1USDT,小于最小金额的充值将不会上账且无法退回。您的充值地址不会经常改变，可以重复充值。请务必确认客户端安全，防止信息被篡改或泄露。
                        </p>
                    </div>
                </div>
                <div>
    ...
    <script>
        var app = new Vue({
            ...
            data:{
                ...
                i:0,
                k:0,
                merchant_info:JSON.parse('<?php echo $merchant_info; ?>'),
                order_info:JSON.parse('<?php echo $order_info; ?>'),
                buyer_usdt:JSON.parse('<?php echo $buyer_usdt; ?>'),
                wx_name:'<?php echo $wx_name; ?>',
                zfb_name:'<?php echo $zfb_name; ?>',
                a:null,
                b:null,
                confirm_pay:1,
                confirm_pay_info:1,
            ...
            methods:{
                ...
                subform(){
                    ...
                    var url = "<?php echo url('home/merchant/uptrade'); ?>";
                ...
                checkouttime(){
                    ...
                    axios.post('<?php echo url("home/merchant/CheckOutTime"); ?>','id='+order_id).then(function(respose){
                        if(respose.data.code == 1){
                            that.close_order = 0;
                            that.confirm_pay = 0;
                            that.confirm_pay_info = 0;
