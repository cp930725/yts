...
class Merchant extends Base{
	public function log(){
        ...
        $lists = Db::name('merchant_log')->field('ml.*, m.name, m.reg_type')->alias('ml')->join('merchant m', 'ml.admin_id=m.id', 'left')
    ...
    //后台充值
        ...
        if(request()->isAjax()){
            ...
            financelog($param['id'],$amount,'后台充值USDT余额',0,session('username'));//添加日志
            
            // dump($user);die;
            //$flag = $member->editMerchant($param);
            $flag = Db::name('merchant')->where('id',$param['id'])->update(['usdt'=>$amount]);
            $data['merchant_id'] = $param['id'];
            $data['num'] = $param['usdt'];
            $data['mum'] = $param['usdt'];
            $data['addtime'] = time();
            $data['status'] = 0;
            $data['confirmations'] = 1;
            Db::name('merchant_recharge')->insert($data);
            if($flag){
            	return json(['code' => 1,'msg' => '充值成功']);
            }else{
            	return json(['code' => 2,'msg' => '充值失败']);
            }
    ...
    public function addresslist(){
        ...
        // dump($map);
            ...
            foreach($lists as $k=>&$v){
                $user=Db::name('merchant')->where(array('id'=>$v['uid']))->find();
                // dump($user);
                $v['addtime'] = getTime($v['addtime']);
    ...
    public function rechargelist(){
        ...
        if($keyuser&&$keyuser!==""){
            ...
            $map['to_address'] = array('in', $address);
        }
        if(!empty($status)){
    ...
    public function orderlist(){
        ...
        if($time_type){
        	if($time_type == 1){
        		...
        		$time24 = $time0 + 86400;
        		$map['think_order_buy.ctime'] = ['between',$time0.','.$time24];
        	}
        	if($time_type == 2){
        		//昨天
        		$time0 = strtotime(date('Y-m-d',time()));
        		$time24 = $time0 - 86400;
        		$map['think_order_buy.ctime'] = ['between',$time24.','.$time0];
        		
        	}
        	if($time_type == 3){
        		//近七天
        		$time0 = strtotime(date('Y-m-d',time()));
        		$time24 = $time0 - 86400 * 7;
        		$map['think_order_buy.ctime'] = ['between',$time24.','.$time0];
        ...
        // dump($lists);
        foreach($lists as $k=>$v){
           $user= Db::name('merchant')->where(array('id'=>$v['sell_id']))->find();
           $accuser= Db::name('merchant')->where(array('id'=>$user['pid']))->find();
            ...
            if($lists[$k]['finished_time']){
             ...
             if(!$lists[$k]['su_reason']){
              ...
              // $str='';
             $sorder=Db::name('ad_sell')->where(array('id'=>$v['sell_sid']))->find();
             if($sorder['pay_method']>0){
                $bank=Db::name('merchant_bankcard')->where(array('id'=>$sorder['pay_method']))->find();
                $lists[$k]['bankinfo']="收款人:".$bank['truename']."<br>开户行:".$bank['c_bank'].$bank['c_bank_detail']."<br>收款账号:".$bank['c_bank_card'];
             }
             if($sorder['pay_method2']>0){
                $zfb=Db::name('merchant_zfb')->where(array('id'=>$sorder['pay_method2']))->find();
                $lists[$k]['zfbinfo']='/uploads/face/'.$zfb['c_bank_detail'];
                // $str.='|<a onclick="showzfb({{d[i].zfbinfo}})">支付宝</a>';
             }
             if($sorder['pay_method3']>0){
                $wx=Db::name('merchant_wx')->where(array('id'=>$sorder['pay_method3']))->find();
                $lists[$k]['wxinfo']='/uploads/face/'.$wx['c_bank_detail'];
             }
             if($sorder['pay_method4']>0){
                $wx=Db::name('merchant_ysf')->where(array('id'=>$sorder['pay_method4']))->find();
                $lists[$k]['ysfinfo']='/uploads/face/'.$wx['c_bank_detail'];
             }
             if($sorder['pay_method5']>0){
                $wx=Db::name('merchant_zfbh5')->where(array('id'=>$sorder['pay_method5']))->find();
                $lists[$k]['zfbh5info']='/uploads/face/'.$wx['c_bank_detail'];
             }
            if($sorder['pay_method6']>0){
                $dmf1=Db::name('merchant_dmf')->where(array('id'=>$sorder['pay_method6']))->find();
                $lists[$k]['dmfh5']='/uploads/face/'.$dmf1['c_bank_detail'];
            }
            if($sorder['pay_method7']>0){
                $bank2=Db::name('merchant_bankcard2')->where(array('id'=>$sorder['pay_method7']))->find();
                $lists[$k]['bank2']='/uploads/face/'.$bank2['c_bank_detail'];
            }
             // dump($str);
              // $lists[$k]['payway'] = $str;
    ...
    public function outorderlist(){
        ...
        // dump($map);
        $member = new MerchantModel();
        $lists = $member->getOrderByWhereNotPage($map);
       
        //文件名称
        $Excel['fileName']="匹配订单记录".date('Y年m月d日-His',time());//or $xlsTitle
        $Excel['cellName']=['A','B','C','D','E','F','G','H'];
        $Excel['H'] = ['A'=>10,'B'=>10,'C'=>40,'D'=>20,'E'=>30,'F'=>30,'G'=>10,'H'=>10];//横向水平宽度
        $Excel['V'] = ['1'=>40,'2'=>26];//纵向垂直高度
        $Excel['sheetTitle']="匹配订单记录";//大标题，自定义
        $Excel['xlsCell']=\app\common\model\Data::headOrder();
        foreach($lists as $k=>$v){
            if($v['status'] == 0){
                $lists[$k]['status'] = '待付款';
            }else if($v['status'] == 1){
            ...
            $lists[$k]['pay_type'] = "";
	         ...
	         if($sorder['pay_method4']>0){
	            //$wx=Db::name('merchant_ysf')->where(array('id'=>$sorder['pay_method4']))->find();
	            //$lists[$k]['ysfinfo']='/uploads/face/'.$wx['c_bank_detail'];
	            $lists[$k]['pay_type'] = "云闪付";
	         }
	         if($sorder['pay_method5']>0){
	            //$wx=Db::name('merchant_ysf')->where(array('id'=>$sorder['pay_method4']))->find();
	            //$lists[$k]['ysfinfo']='/uploads/face/'.$wx['c_bank_detail'];
	            $lists[$k]['pay_type'] = "支付宝h5";
	         }
            if($sorder['pay_method6']>0){
                //$wx=Db::name('merchant_ysf')->where(array('id'=>$sorder['pay_method4']))->find();
                //$lists[$k]['ysfinfo']='/uploads/face/'.$wx['c_bank_detail'];
                $lists[$k]['pay_type'] = "当面付";
            }
            if($sorder['pay_method7']>0){
                //$wx=Db::name('merchant_ysf')->where(array('id'=>$sorder['pay_method4']))->find();
                //$lists[$k]['ysfinfo']='/uploads/face/'.$wx['c_bank_detail'];
        ...
        \app\common\model\PHPExcel::excelPut($Excel,$lists);
    }
    
    //新加订单导出
    public function outorderlistbuy(){
        /* [
         ['id','序号'],
         ['ordersn','订单号'],
         ['num','提币USDT'],
        ...
        if($time_type){
        	if($time_type == 1){
        		...
        		$time0 = strtotime(date('Y-m-d',time()));
        		$time24 = $time0 + 86400;
        		$map['a.ctime'] = ['between',$time0.','.$time24];
        	}
        	if($time_type == 2){
        		//昨天
        		$time0 = strtotime(date('Y-m-d',time()));
        		$time24 = $time0 - 86400;
        		$map['a.ctime'] = ['between',$time24.','.$time0];
        ...
        // dump($map);
        $member = new MerchantModel();
        $lists = $member->getOrderBuyByWhereNotPage($map);
       
        //文件名称
        $Excel['fileName']="匹配订单记录".date('Y年m月d日-His',time());//or $xlsTitle
        $Excel['cellName']=['A','B','C','D','E','F','G','H','I'];
        $Excel['H'] = ['A'=>10,'B'=>10,'C'=>40,'D'=>20,'E'=>30,'F'=>30,'G'=>10,'H'=>10,'I'=>10];//横向水平宽度
        $Excel['V'] = ['1'=>40,'2'=>26];//纵向垂直高度
        $Excel['sheetTitle']="匹配订单记录";//大标题，自定义
        $Excel['xlsCell']=\app\common\model\Data::headOrderBuy();
        foreach($lists as $k=>$v){
            if($v['status'] == 0){
                $lists[$k]['status'] = '待付款';
            }else if($v['status'] == 1){
	...
	public function orderlistbuy(){
        ...
        foreach($lists as $k=>$v){
           ...
           if($lists[$k]['finished_time']){
             ...
             if($v['pay']>0){
                $bank=Db::name('merchant_bankcard')->where(array('id'=>$v['pay']))->find();
                $lists[$k]['bankinfo']="收款人:".$bank['truename']."<br>开户行:".$bank['c_bank'].$bank['c_bank_detail']."<br>收款账号:".$bank['c_bank_card'];
             }
             if($v['pay2']>0){
                $zfb=Db::name('merchant_zfb')->where(array('id'=>$v['pay2']))->find();
                $lists[$k]['zfbinfo']='/uploads/face/'.$zfb['c_bank_detail'];
                // $str.='|<a onclick="showzfb({{d[i].zfbinfo}})">支付宝</a>';
             }
             if($v['pay3']>0){
                $wx=Db::name('merchant_wx')->where(array('id'=>$v['pay3']))->find();
                $lists[$k]['wxinfo']='/uploads/face/'.$wx['c_bank_detail'];
             }
        }
        // dump($lists);
	...
	public function ssfailbuy(){
        ...
        try{
            ...
            if($rs1 && $rs2 && $rs3){
               // financelog($orderinfo['sell_id'],($orderinfo['deal_num']+$orderinfo['fee']),'卖出冻结USDT_释放_3',1,session('username'));//添加日志
                financelog($orderinfo['sell_id'],$mum,'卖出USDT_释放_3',0,session('username'));//添加日志
                // 提交事务
                Db::commit();
                $this->success('操作成功');
            }else{
