...
class Login extends Base{
    ...
    public function register(){
        ...
        if(!session('uid')){
            ...
            header('Location: /merchant/index');die;
        }
    }
...
//exit;
    ...
    public function sendPhoneCode(){
         if(request()->isPost()){
           ...
           if(preg_match('/^13[\d]{9}$|^14[5,7]{1}\d{8}$|^15[^4]{1}\d{8}$|^16[\d]{9}$|^17[\d]{9}$|^18[\d]{9}$|^19[\d]{9}$/',$mobile)){
                $code = rand(100000,999999);
//                $content = '您的验证码为:'.$code.'.验证码有效时间为2分钟.如不是本人操作,请忽略.';
                // $result = 2;
                $result = send_moble($mobile,[$code, 10], '796923');
                if($result){
